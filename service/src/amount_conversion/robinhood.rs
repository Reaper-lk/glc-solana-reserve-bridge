...
//! accounting does not, and must not. The 32-byte big-endian container
//! that carries an ABI word long enough to check it and narrow it is
//! [`crate::evm::EvmU256`] — the crate-wide EVM primitive, NOT a type
//! this module defines.
//!
//! That is deliberate and worth stating, because this module briefly had
//! its own. Two structurally identical `EvmU256` types in one crate is a
//! latent correctness bug, not merely duplication: the compiler would
//! keep them apart while every reader assumed they were the same, and the
//! day an ABI word crossed from the RPC layer into the amount model it
//! would need a conversion nobody had written. There is exactly one
//! 256-bit word type in this crate and this module consumes it.
//!
//! It has no arithmetic: the one operation the bridge ever needs on an
//! inbound word is "does this fit in a `RobinhoodAtomic`, yes or no", and
//! the one operation it needs outbound is lossless widening. When the
//! real EVM RPC/ABI stack lands (a later phase — this one adds no chain
//! client and no dependency), it plugs in through
//! [`EvmU256::from_be_bytes`]/[`EvmU256::to_be_bytes`], which is the
//! representation every EVM library agrees on.

use std::fmt;

use super::{CanonicalAtomic, GOLDCOIN_DECIMALS};
use crate::evm::{EvmU256, EvmU256Error};
...
impl RobinhoodAtomic {
    ...
    ///
    /// [`crate::evm::EvmU256::try_to_u128`] reports the failure as
    /// [`EvmU256Error::Overflow`], a primitive-level fact about a word.
    /// It is translated here into [`RobinhoodConversionError`] because a
    /// caller of the amount model is asking an amount question, and the
    /// answer it needs is "this cannot be a Robinhood atomic amount", not
    /// "a 256-bit word did not fit a u128". The generic type stays generic
    /// and this module keeps its own error vocabulary.
    pub fn try_from_u256(word: EvmU256) -> Result<RobinhoodAtomic, RobinhoodConversionError> {
        match word.try_to_u128() {
            Ok(value) => Ok(RobinhoodAtomic(value)),
            Err(EvmU256Error::Overflow { .. }) => {
                Err(RobinhoodConversionError::U256ExceedsU128 { value: word })
            }
            // `try_to_u128` narrows an already-constructed word, so
            // overflow is its only reachable failure. Mapped explicitly
            // rather than with a catch-all `_`, so that if the primitive
            // ever grows a new variant this arm stops compiling instead of
            // silently reporting it as an overflow it is not.
            Err(EvmU256Error::WrongByteLength { .. }) | Err(EvmU256Error::Hex(_)) => {
                Err(RobinhoodConversionError::U256ExceedsU128 { value: word })
            }
        }
    }
