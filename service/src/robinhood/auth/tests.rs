...
mod golden {
    //! The cross-language fixture, read at COMPILE time from the exact
    //! path the Foundry suite reads at run time.
    //!
    //! `include_str!` rather than a runtime file read: the test binary
    //! then carries the fixture's bytes, so this test cannot pass because
    //! a file happened to be missing or because a path resolved
    //! somewhere unexpected — a wrong path is a build failure, and a
    //! wrong value is a test failure.

    pub const FIXTURE: &str =
        include_str!("../../../../contracts/test/fixtures/eip712-golden.json");
    ...
    /// rendered as the string it appears as in the file.
    ///
    /// Parsed with `serde_json` — already a dependency — rather than
    /// scanned textually: two of the three vectors use the same leaf key
    /// (`structHash`) under different parents, so a leaf-only lookup
    /// would silently compare against whichever one it found first.
    ///
    /// A path that resolves to nothing panics. A fixture key renamed on
    /// the Solidity side therefore fails loudly here instead of quietly
    /// asserting nothing.
    pub fn get(path: &str) -> String {
        let root: serde_json::Value =
            serde_json::from_str(FIXTURE).expect("the golden fixture must be valid JSON");
        // `inputs` is a flat block of documented parameters; the digests
        // sit at the top level. Try the literal path first, then under
        // `inputs`, so a caller writes `"expiry"` rather than
        // `"inputs.expiry"` for the half that lives there.
