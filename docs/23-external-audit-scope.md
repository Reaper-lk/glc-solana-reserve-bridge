...
`programs/glc-reserve-bridge/src/instructions/initialize.rs` (upgrade-authority-gated `initialize`) and `programs/glc-reserve-bridge/src/instructions/upgrade_timelock.rs` (the timelocked-upgrade mechanism: `accept_upgrade_authority`/`propose_upgrade`/`execute_upgrade`/`cancel_upgrade`), docs/12-management-decisions.md item 3. **This is a first-class review area, not a footnote, regardless of the mechanism now existing**: `execute_upgrade` performs a real `bpf_loader_upgradeable::upgrade` CPI once a program-owned PDA holds real upgrade authority — an attacker (or a compromised single admin key, since propose/cancel are admin-gated, not threshold-gated) who can propose and let a timelock elapse can eventually replace the entire program's logic, bypassing every other control this document set describes. Review: (a) whether `invoke_signed`'s account/seed handling in `accept_upgrade_authority`/`execute_upgrade` is correct under adversarial account substitution (a wrong or attacker-supplied `buffer`/`program_data`/`upgrade_authority_pda` account); (b) whether the admin-gated propose/cancel authorization is the right posture for something this powerful, versus threshold-gating it the way attestation-key rotation is gated; (c) whether the timelock delay itself (`upgrade_timelock_seconds`, a config value, not yet set to a production number) is long enough to be a meaningful operator-reaction window. **Still an open management decision, unaffected by the mechanism's existence**: whether this timelocked-PDA posture (option (c)) is the one actually used at deploy time versus full threshold custody (a) or revoking upgradeability entirely (b) — the mechanism ships inert (`accept_upgrade_authority` was never called with a real key) until that decision is made and acted on. Flag any code-level finding here as blocking final sign-off regardless of quality elsewhere (see §5); flag the posture decision itself as a separate, non-code finding for management.
...
| Program upgrade authority final control | Management decision between immutable, threshold-custodied, or timelocked upgrade authority; **the timelock mechanism (option (c)) is now built and tested, but not activated** — the program is deployed upgradeable with a single external authority key until `accept_upgrade_authority` is deliberately called with that real key, which has not happened and is not this repository's decision to make | docs/12 item 3 |
