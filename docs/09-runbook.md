...
3. **Automatic shaping** (`goldcoin::liquidity::run_shaping_tick`, wired as `Orchestrator::tick_utxo_liquidity_shaping` after the payout pass): at most ONE split-shaped action per tick — first resume any split stuck in `Built`/`Signed` (restart safety; `Built` re-signs via `signing::goldcoin_split::RecoverySplitSource`, which proves the reconstructed plan serializes byte-identically to the persisted unsigned transaction before any signature is produced; `Signed` re-broadcasts its exact stored bytes via `goldcoin::split_recovery`); otherwise, only while the payout-ready mature pool (Available UTXOs of at least half the chunk target) is below `utxo_shaping_target_available_count` AND no previous split's chunks are still maturing, split the largest mature, unreserved, never-before-split root-vault UTXO of at least `utxo_shaping_min_source_atomic` — through the IDENTICAL independent 2-of-3 signing path and non-overridable reserve-floor refusal `glc-admin split-vault-utxo` uses. A very large source splits into at most `utxo_shaping_max_outputs_per_split` correspondingly larger chunks (each itself a later candidate, consumed only if the pool thins again) — gradual shaping, never one enormous transaction, and never a flood of tiny UTXOs (`plan_split` already refuses sub-2-chunk splits; chunk sizing never goes below the canonical target).
4. **Solvency-aligned split safety check** (`signing::goldcoin_split`, both fresh and recovery sources, plus the CLI's own pre-check): the reserve-floor refusal is now `balance - fee >= protected_minimum + pending_obligations`, replacing `balance - source_amount >= floor`. A split never removes value from the vault's custody — every chunk output pays the vault's own script and is ledger-tracked from the instant of broadcast — so only the network fee genuinely leaves. The old formula pretended the whole source had left, which deadlocked the exact bootstrap scenario shaping exists for: a vault whose dominant liquidity IS one oversized deposit could never be restructured at all. The check remains non-overridable and runs independently in every signer.
5. **Atomic split broadcast effects** (`Ledger::record_vault_utxo_split_broadcast_effects`, applied by both the daemon tick and the CLI, fresh and recovered paths alike): the instant a split broadcasts, one transaction marks the source `Spent` (coin selection can never race the already-spent outpoint) and inserts every chunk as an `Unconfirmed` `vault_utxos` row — so `own_unconfirmed_change_atomic` (now matching split txids as well as payout txids) explains the mature-balance dip immediately, reconciliation's hard invariant never misreads the service's own split as unexplained loss, and a broadcast split's fee joins payout fees in the permanent-departures term.
...
Config (all under `[goldcoin]`): `utxo_shaping_enabled` (default `true`), `utxo_shaping_target_available_count` (default 15 — matches `utxo_pool_warning_count`, so shaping starts rebuilding exactly when the warning gauge would show), `utxo_shaping_min_source_atomic` (default `4 * change_fanout_target_atomic` = 20,000 GLC; validated `>= 2x` the chunk target), `utxo_shaping_max_outputs_per_split` (default 25). The daemon logs all effective values at startup for cross-signer drift diagnosis.

### Regression coverage

`service/tests/utxo_liquidity_autoshaping.rs` drives the real production code paths end to end (real ledger, real independent 2-of-3 split signing, real selection/fan-out, accept-all broadcast double): **A** — one 1,000,000 GLC deposit funds six cycles of repeated 20,000 GLC payouts with zero manual splits, no `TooManyInputs`, no outpoint reuse, and reconciliation clean throughout; **J** — the production bootstrap (the deposit IS the entire reserve, admission floor 10) parks on the count floor, shapes anyway under the solvency-aligned check, and self-recovers after one maturity window; **B** — the incident's exact fragmented pool shape selects within `max_inputs`; **D/E/F** — unconfirmed internal change and sub-6-conf external deposits are never selectable, and become selectable exactly at maturity; **G** — concurrent reservation cannot double-select an input; **H/H2** — restart resumes `Signed` splits by stored bytes and `Built` splits by verified re-signing; **I/I2** — a healthy pool produces no self-transactions, and shaping never stacks a second split while chunks mature. The selector's feasibility-completeness and the `max_inputs = 25` cost decision are pinned in `service/src/goldcoin/coin.rs`'s own tests.
