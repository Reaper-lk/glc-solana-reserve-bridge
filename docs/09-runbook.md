...
**This is not just a worked example — it is empirically verified** by `service/tests/utxo_liquidity_production_tuning.rs::test_prod_defaults_floor_8_breaches_before_backpressure_engages`, run against the literal shipped defaults (`utxo_pool_min_available_count = 8`, `vault_min_confirmations = 6`, `fee_rate_per_kb = 100_000` — the real pilot-template fee rate, not a toy value). At `floor = 8` against this exact vault shape: obligations 0-11 (12 total) are admitted and finalized; reconciling before obligation 12 finds the hard invariant already breached (observed mature balance 38,160 GLC < protected_minimum 20,000 + pending_obligations 22,560), and the direction auto-pauses. Because the hard-invariant check runs before `fold_sol_deposit`'s own count-based check on the very same obligation, **the recorded `ManualReview` reason for every parked obligation from index 12 onward is `reserve_paused_at_fold`, never `utxo_liquidity_low_at_fold`** — count-based backpressure never gets the chance to be the thing that actually caught it. Both the breach and the parking are still fail-safe (no fund loss, no double-spend, no data corruption — the same test asserts exact conservation throughout), but the intended "engage before exhaustion" behavior did not happen at the shipped default for this vault shape.
