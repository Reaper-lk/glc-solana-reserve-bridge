...
3. **The split lifecycle** (`goldcoin::liquidity`, schema v16). Every split — automatic or CLI-initiated — moves through ONE persisted state machine: `Built -> Signed -> Broadcast -> Confirmed`, with `Abandoned` reachable from any non-terminal state. The load-bearing properties:
   ...
   - **`Broadcast` is driven, not abandoned to fate**: each tick, the first observed confirmation (via this service's own synced chain view) marks the split `Confirmed` (terminal); a split the node no longer knows (mempool eviction, e.g. a node restart) is re-broadcast from its exact stored bytes; a re-broadcast refused for missing inputs (a conflicting spend won) is `Abandoned` — its phantom chunk rows are marked `Spent` in the same transaction so accounting never keeps explaining value that cannot arrive. A transport-level RPC failure defers to the next tick — an unreachable node never triggers an abandonment.
   - **`Abandoned` is the release valve**: the audit row is kept forever, but the source-outpoint uniqueness index is partial (`WHERE state != 'Abandoned'`), so a legitimately returned outpoint can be split afresh. A `Built`/`Signed` split whose source is no longer `Available` is abandoned instead of erroring forever; a never-broadcast `Signed` split re-checks the reserve floor against CURRENT state before its bytes ever reach the network. No state can permanently wedge shaping, and no recovery path involves SQLite.
   ...
   - **The CLI is the same code**: `glc-admin split-vault-utxo` calls the identical `goldcoin::liquidity` functions (`execute_fresh_split`, `resume_pending_split`, `maintain_broadcast_splits`) — it resumes a pending split instead of falsely reporting it done, and there is no CLI action that can strand lifecycle state.
