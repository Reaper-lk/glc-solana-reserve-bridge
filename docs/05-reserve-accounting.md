...
> support.md) and the 3% bridge fee (docs/20-bridge-fee.md). The
...
>
> **Confirmed-liquidity admission safety buffer (2026-09-02)**: SolToGlc admission additionally requires `total_reserve_balance >= protected_minimum + reserved_liquidity + net_destination_atomic + admission_safety_buffer` (250,000 GLC in production), with a direction-wide hysteresis gate that closes below the buffer and reopens only at 350,000 GLC. The buffer is an ADMISSION gate layered on top of `protected_minimum` — it is never a term in the hard invariant or in `available_capacity`, both of which keep the formulas defined in this document exactly as written. A reserve can therefore be entirely solvent while admission is closed; that is the intended state. Immature payout change is deliberately not counted as headroom. See [09-runbook.md](09-runbook.md)'s "Confirmed-liquidity admission safety buffer" section.
...
those settlements, exactly — but NOT 1:1 gross-for-gross since the 3%
bridge fee round: it equals the sum of those deposits' NET amounts (gross
minus the 3% fee), with no accumulated rounding beyond the documented
