...
| version | taken by |
|---|---|
| v18 | confirmed-liquidity admission safety buffer, PR #55 (`9dbc692`) |
| v19, v20 | subsequent upstream work merged into `main` |
| v21 | chain-qualified source obligation identity (see below) |

v22 is the next free number, re-confirmed at integration time against every
local and remote ref. Nothing about this change touches any of that work;
only our number moves.
...
INSERT OR IGNORE INTO bridge_routes VALUES ('SolToRhn','solana','robinhood',0,NULL,:now);
INSERT OR IGNORE INTO bridge_routes VALUES ('RhnToSol','robinhood','solana',0,NULL,:now);
```

Six rows, not four: the custody contract models `SolToRhn` and `RhnToSol`
structurally and ships them disabled, so the ledger must be able to record
their disabled state rather than leave them unrepresented. All four
Robinhood rows seed `enabled = 0`.
...
v22 was free across every local and remote ref when this integration was
assembled: no branch defines `apply_v22` and none references `bridge_routes`
outside this work. That is exactly what the previous revision of this
section said about v19, and v19 is now taken — so re-check again when Phase
2 actually writes it. Two standing reasons:
...
2. ~~**`bridge_requests.direction` CHECK widening + `source_chain` column**~~
   — **DONE.** This landed as **v21** (chain-qualified source obligation
   identity), which rebuilds `bridge_requests` with a `source_chain` column
   and a `source_contract` qualifier, so obligation indices are namespaced
   per chain-and-contract instead of sharing one namespace. The collision
   this section warned about — a Robinhood obligation 0 colliding with a
   Solana obligation 0 — is resolved. It was correctly identified here as
   **blocking** before any Robinhood route is enabled, and it is no longer
   blocking.

The remaining item is numbered **relative to** the `bridge_routes`
migration, not fixed: if `bridge_routes` lands at v22 the `reserve_ledger`
rebuild becomes v23. Both numbers are provisional until Phase 2 re-runs the
collision scan.
